...
function PopupWithForm(props){
    ...
    console.log("🚀 ~ PopupWithForm ~ props:", props)
        ...
        <div className={`popup popup_type_${props.name} ${isOpen}`}>
            <div className="popup__form">
                <button className="button button_action_close" aria-label="Cerrar ventana">
                    <img src={icon__close} className="button__close-image" alt="Cerrar ventana" onClick={props.onClose} />
                </button>
                <div className="popup__content" >
                    <form className="form" name={`${props.name}`}>
                        <h3 className="form__title">{props.title}</h3>
                        {props.children}
                        <button type="submit" className="form__button" aria-label="Guardar"></button>
                    </form>
                </div>
            </div>
        </div>
